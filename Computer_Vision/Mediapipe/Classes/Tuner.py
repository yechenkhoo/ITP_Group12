...
class CVTuner(kt.BayesianOptimization):
    ...
    # def run_trial(self, trial, X, y, groups, epochs=200, **kwargs):
    #     """
    #     Run a single trial with cross-validation using GroupKFold
    #     """
    #     hp = trial.hyperparameters

    #     # Use GroupKFold to keep groups together in each fold
    ...
    #         print(f"  Fold {fold + 1}/{self.cv_folds}")
    #         # Print group info for this fold
    ...
    #         # Evaluate on validation set
    #         val_loss, val_accuracy = model.evaluate(X_val_fold, y_val_fold, verbose=0)
    #         print(f"    Fold {fold + 1} val_accuracy: {val_accuracy:.4f}")
    #         cv_scores.append(val_accuracy)
    #         cv_losses.append(val_loss)

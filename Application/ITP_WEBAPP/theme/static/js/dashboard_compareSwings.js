document.addEventListener('DOMContentLoaded', () => {
  ...
  //
    ...
    // Call function to update table headers
    updateTableHeaders(window.video1Title, window.video2Title); // NEW LINE
  ...
  // NEW FUNCTION: Update table headers
  function updateTableHeaders(leftTitle, rightTitle) {
    const headerVideo1 = document.querySelector('.tab-content:not(.hidden) #header-video1-title');
    const headerVideo2 = document.querySelector('.tab-content:not(.hidden) #header-video2-title');

    if (headerVideo1) {
      headerVideo1.textContent = leftTitle;
    }
    if (headerVideo2) {
      headerVideo2.textContent = rightTitle;
    }
  }
  ...
  document.querySelectorAll('[data-tab]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      updateDatasets(); // This now calls updateTableHeaders internally.
      ...
      if (window.setupComparePageEventListeners) {
            window.setupComparePageEventListeners();
        }
    });
  });
